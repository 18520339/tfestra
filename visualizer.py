...
def colorize_pred_text(label, pred_label, fontdict, width, height):
    ...
    while start <= end < label_length:
        ...
        if label[start:end + 1] in pred_label[pred_start:pred_length]:
            ...
            plt.text(width, text_y, actual_token, fontdict=fontdict)
        else:
            if end < pred_length and end + 1 < label_length and pred_label[end] == label[end + 1]:
                ...
                plt.text(width * 2, text_y, actual_token, fontdict=fontdict)
            else: 
                fontdict['color'] = 'gray'
                plt.text(width, text_y, actual_token, fontdict=fontdict)
